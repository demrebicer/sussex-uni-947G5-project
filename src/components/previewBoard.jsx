...
const GridComponentBase = ({ data, colors }) => {
  ...
  const rows = Array.from({ length: 5 }, (_, rowIndex) => data.slice(rowIndex * 11, (rowIndex + 1) * 11));
      ...
      {rows.map((row, rowIndex) => (
        <div key={rowIndex} className="flex">
          {row.map((id, colIndex) => {
            ...
            return <div key={colIndex} className={`w-4 h-4 ${colorClass} border-1 border-white border-opacity-20`}></div>;
          })}
        </div>
      ))}
    </div>
